---
description: Maintainer guide for @youdotcom-oss/ai-sdk-plugin.
...
Vercel AI SDK tools for the hosted You.com MCP server.
...
- `publish-ai-sdk-plugin.yml` publishes to npm with provenance and creates a GitHub release
- `droid-review.yml` handles automated PR review
- `semgrep-ci.yml` runs security scanning

### Publishing Rules

- Package releases are triggered through `publish-ai-sdk-plugin.yml` workflow dispatch.
- The workflow computes the next version, updates `package.json`, publishes to npm, and creates a GitHub release tag in the form `ai-sdk-plugin@v{version}`.
